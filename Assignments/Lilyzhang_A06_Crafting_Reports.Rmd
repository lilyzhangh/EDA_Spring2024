...
# Define metadata
EPA.Air.Quality <- data.frame(
  Item = c("Source", "Data", "Filename"),
  Value = c("EPA Air Quality System (AQS)", "2018-2019", "EPAair_O3_PM25_NC1819_Processed.csv")
)
# Report the summary
knitr::kable(EPA.Air.Quality, align = c("r", "l"))
```
...
```{r, fig.align='left', fig.cap = "Figure 1: 2018 Ozone values by month", echo=FALSE, message=FALSE, error=FALSE, warning=FALSE, fig.width=6, fig.height=4}
library(reporter)
library(magrittr)
#\usepackage{siunitx}
# Filter data for records collected in 2018
EPA.2018 <- filter(epa_data, year(Date) == 2018)

# Create a boxplot of Ozone levels by month for 2018
ggplot(EPA.2018, aes(x = month(Date, label = TRUE), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", 
...
```{r, fig.align='left', echo=FALSE, error=FALSE, fig.cap="Figure 2: 2019 Ozone values by month", message=FALSE, warning=FALSE, fig.width=6, fig.height=4}
# Filter data for records collected in 2019
EPA.2019 <- filter(epa_data, year(Date) == 2019)

# Create a boxplot of Ozone levels by month for 2019
ggplot(EPA.2019, aes(x = month(Date, label = TRUE), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Month", 
       y = expression(paste('Concentration (',mu,'g/',m^3,')')),
...
## Git Repository
https://github.com/lilyzhangh/EDA_Spring2024
